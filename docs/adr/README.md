...
- [0001 — Freestanding font-data core, separate from the stdlib library face](0001-freestanding-font-data-core.md) — why kashi ships a no-stdlib `src/font_data.cyr` the agnos kernel can `include` directly, distinct from the stdlib-using `src/lib.cyr` library face. Amended 2026-08-17 (1.0.6): the library face is consumed as the `cyrius distlib` bundle `dist/kashi.cyr`, tracked and released since 1.0.10.
...
- [0009 — PCF import](0009-pcf-import.md) — heapless binary parser in `src/font_pcf.cyr` for X11 PCF compiled bitmap fonts (TOC-driven, multi-table). Strict-uniform-metrics analog of ADR 0008; all four byte-order × bit-order combos with canonicalize-on-load (scan-unit byte swap + bit reverse); both compressed and uncompressed metrics; required tables `PCF_METRICS` / `PCF_BITMAPS` / `PCF_BDF_ENCODINGS`. `KIND=4` in the parsed-header struct.
- [0010 — PSF u-variant: sidecar Unicode tables + strict UTF-8](0010-psf-u-variant.md) — four `kashi_attach_unicode_table*` APIs to attach an externally-supplied codepoint map to an already-loaded font (binary PSF1/PSF2 table bytes or text `psfgettable` format). Closes the 0.6.0 audit F2 with overlong-UTF-8 rejection per RFC 3629 in `kashi_psf_uni_token`. Drops the "larger table-walk caps" roadmap item as a non-issue on re-examination.
